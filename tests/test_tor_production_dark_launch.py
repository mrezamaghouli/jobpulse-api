...
_ALLOWED_TOR_ENABLED_SETTERS = {
    ...
    ".github/workflows/production-split-release-recovery.yml",  # Phase 4H: read-only pre/post invariant checks (must stay 'false' throughout, via `docker exec ... printenv TOR_ENABLED`), invoked only by the manual workflow_dispatch-only production-split-release-recovery.yml, never dispatched by this task; this runner's only mutation is `git reset --hard` on production git, never a container recreation, and it never sets TOR_ENABLED=true
    ".github/workflows/production-api-reference-normalization.yml",  # Phase 4M: read-only TOR_ENABLED invariant checks only (via `docker exec ... printenv TOR_ENABLED` and a narrow `docker inspect --format '{{range .Config.Env}}...'` parse), invoked only by the manual workflow_dispatch-only production-api-reference-normalization.yml, never dispatched by this task; it never sets TOR_ENABLED=true and never contacts Tor/ControlPort/NEWNYM -- its only mutation recreates the api container with a canonical image reference (same underlying bytes), never touching Tor configuration
}
