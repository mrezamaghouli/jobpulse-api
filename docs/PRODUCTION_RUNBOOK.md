...
- The later successful run `34679402811` completed the full
  ...
  ```
  -- the historical scanner reported exactly one discovered launcher,
  the authoritative own-crontab launcher, and confirmed both lock
  paths. That historical scanner's own systemd-source enumeration used
  fail-open (`|| true`) error handling, so this is evidence of what was
  discovered, not a mathematical proof that no other systemd launcher
  could have existed. The current Phase 4H recovery runner strengthens
  alternate systemd-source verification beyond that historical
  limitation (see the fail-closed correction below).
...
2. **Proves no alternate scheduler source has since started declaring a
   launcher.** The historical scanner (run `34679402811`) discovered
   exactly one launcher, but its own systemd enumeration used fail-open
   (`|| true`) error handling, so it never conclusively ruled out an
   alternate systemd source -- see "Systemd fail-closed correction"
   below. This runner now checks `/etc/crontab`, every regular file
   directly under `/etc/cron.d`, root's crontab (`timeout 5 sudo -n
   crontab -u root -l` -- non-interactive, never prompts, never
   installs a sudo rule; `sudo -n`'s own "no crontab for root" is the
   accepted default state here, since root having no crontab was never
   expected to change), and every systemd service unit's *effective*
   configuration (`systemctl list-unit-files` + `systemctl show
   --property=ExecStart,Environment,EnvironmentFiles,FragmentPath,DropInPaths`,
   fail-closed -- see "Systemd fail-closed correction" below) for ANY
   mention of `run_collection_cycle_safe.sh`,
   `JOBPULSE_COLLECTION_ROOT`, or `JOBPULSE_COLLECTION_CYCLE_LOCK_PATH`.
   This is deliberately an absence-only check -- it never parses
   launcher grammar from these other sources (that full parser lives
   only in the general-purpose upgrade runner) -- any match at all is
   treated as unexpected drift and fails closed. Only bounded
   status/labels are ever printed; full file contents (including raw
   crontab output on a generic read failure) are never dumped.
...
A second independent review found the systemd alternate-source drift
check in step 2 above was itself fail-open: it silently skipped when
`systemctl` was missing, swallowed `systemctl list-unit-files` and
`systemctl show` failures/timeouts with `|| true`, and only inspected
raw `FragmentPath`/`DropInPaths` file text via a candidate-then-`cat`
mechanism that silently skipped unreadable paths -- none of which is
acceptable for a production recovery runner's only defense against an
alternate systemd launcher. This is now fixed:

- `command -v systemctl` is mandatory -- if `systemctl` is unavailable,
  the runner fails BEFORE any mutation rather than silently skipping
  the check.
- `systemctl list-unit-files --type=service --no-legend --no-pager` is
  bounded (`timeout 5`) with its exit status captured explicitly
  (`... 2>&1)" || UNIT_LIST_STATUS=$?`, never `|| true`); a nonzero
  status fails closed.
- Every enumerated unit gets a bounded, explicit-exit-status
  `systemctl show <unit> --property=ExecStart,Environment,EnvironmentFiles,FragmentPath,DropInPaths --no-pager`
  inspection of its *effective* configuration (not just the raw
  fragment file text); failure, timeout, or an empty/unusable result
  fails closed. The separate `systemctl cat` + raw-file-grep candidate
  mechanism was removed entirely -- effective `systemctl show` output
  is simpler and directly sufficient to prove absence of the relevant
  runtime configuration.
- Drift is detected directly against each unit's effective `systemctl
  show` output for `run_collection_cycle_safe.sh`,
  `JOBPULSE_COLLECTION_ROOT`, or `JOBPULSE_COLLECTION_CYCLE_LOCK_PATH`
  -- no systemd grammar parsing required.
- As with the crontab checks, only bounded status codes and
  source/action labels are ever reported on failure -- the complete raw
  `systemctl` output is never printed.
